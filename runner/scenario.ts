...
export interface Given {
  readonly runtime?: { readonly clock?: { readonly start: number; readonly stepMs: number } }
...
export type Step =
  | { readonly send: { readonly key: string; readonly trigger: TriggerSpec }; readonly await?: boolean }
  | { readonly drain: true }
...
export function isSendStep(step: Step): step is Extract<Step, { send: unknown }> {
  return "send" in step
}
...
export function loadScenarios(dir: string = SCENARIOS_DIR): Scenario[] {
  ...
  for (const level of LEVELS) {
    ...
    for (const name of names) {
      ...
      const expectedId = `${level}/${name.replace(/\.json$/, "")}`

      if (scenario.id !== expectedId) {
        throw new Error(`scenario at ${path} declares id "${scenario.id}", expected "${expectedId}"`)
      }
      if (scenario.level !== level) {
        throw new Error(`scenario ${scenario.id} declares level "${scenario.level}" but sits in ${level}/`)
      }
