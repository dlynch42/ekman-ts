...
export function summarize(results: readonly ScenarioResult[]): Report {
  const claimed = results.filter((r) => CLAIMED_LEVELS.includes(r.scenario.level))
...
export function format(report: Report): string {
  ...
  for (const level of LEVELS) {
    ...
    if (!claimed) {
      const note = results.length === 0 ? "no scenarios yet" : `${results.length} scenarios, not run`
    ...
    lines.push(`  ${level}: ${results.length - failed}/${results.length} passing`)
