...
describe("addressing", () => {
  it("rejects a malformed key", async () => {
    const ekman = new Ekman({ entities: [orders] })
    expect(await code(ekman.send("orders:bad key", { type: "approve" }))).toBe("INVALID_KEY")
    expect(await code(ekman.send("orders", { type: "approve" }))).toBe("INVALID_KEY")
  })

  it("rejects a key whose first segment names no entity", async () => {
    const ekman = new Ekman({ entities: [orders] })
    expect(await code(ekman.send("invoices:1", { type: "approve" }))).toBe("UNKNOWN_ENTITY")
  })
  ...
  it("builds keys through the handle", async () => {
    ...
    const result = await ekman.entities.orders.send("abc", { type: "approve", actor: "amy" })
    expect(result.key).toBe("orders:abc")
  })

  it("rejects a trigger with no type", async () => {
    const ekman = new Ekman({ entities: [orders] })
    expect(await code(ekman.send("orders:1", {} as { type: string }))).toBe("UNKNOWN_TRIGGER")
  })
})
...
describe("initialization", () => {
  it("starts an instance in the declared initial state with empty values", async () => {
    ...
    expect(init).toMatchObject({ from: null, to: "pending", seq: 0, values: {} })
  })
  ...
  it("records initialization as the one event whose from is null", async () => {
    ...
    const nullFrom = transitions(ekman.history("orders:1")).filter((e) => e.from === null)
  ...
  it("starts from declared initial values", async () => {
    const withValues = defineEntity("carts", {
      ...
      states: { open: (cart) => stay({ ...cart.values, total: cart.values.total + 1 }) },
    })
...
describe("handler results", () => {
  it("commits a transitionTo", async () => {
    const ekman = new Ekman({ entities: [orders] })
    const result = await ekman.send("orders:1", { type: "approve", actor: "amy" })
    ...
    expect(ekman.inspect("orders:1")).toMatchObject({ state: "approved", seq: 1 })
  })
  ...
  it("commits a stay without changing state", async () => {
    ...
    expect(result).toMatchObject({ state: "approved", seq: 2, values: { by: "amy", seen: true } })
  })

  it("leaves the instance untouched on fail", async () => {
    const ekman = new Ekman({ entities: [orders] })
    expect(await code(ekman.send("orders:1", { type: "cancel" }))).toBe("HANDLER_FAILED")

    expect(ekman.inspect("orders:1")).toMatchObject({ state: "pending", seq: 0, values: {} })
  ...
  it("treats a thrown error as fail", async () => {
    ...
    expect(await code(ekman.send("thrower:1", { type: "go" }))).toBe("HANDLER_FAILED")
  ...
  it("treats a handler that returns a non-result as fail", async () => {
    ...
    expect(await code(ekman.send("wrong:1", { type: "go" }))).toBe("HANDLER_FAILED")
  })
...
describe("error handlers", () => {
  it("can turn a failure into a committed transition", async () => {
    const recovering = defineEntity("recovering", {
      ...
      onError: { RangeError: (instance) => transitionTo("failed", { ...instance.values }) },
    })
  ...
  it("still rejects when the error handler itself throws", async () => {
    ...
    expect(await code(ekman.entities.worse.send("1", { type: "go" }))).toBe("HANDLER_FAILED")
    // The original failure is the one reported; the instance never moved.
    expect(ekman.entities.worse.inspect("1")).toMatchObject({ state: "a", seq: 0 })
  })
  ...
  it("still rejects when the error handler also fails", async () => {
    ...
    expect(await code(ekman.entities.stubborn.send("1", { type: "go" }))).toBe("HANDLER_FAILED")
    expect(ekman.entities.stubborn.inspect("1")).toMatchObject({ state: "a", seq: 0 })
  })
})
...
describe("unknown policy", () => {
  it("refuses a trigger in a state with no handler, and records it", async () => {
    ...
    expect(await code(ekman.send("dead:1", { type: "go" }))).toBe("UNKNOWN_STATE")

    const rejected = ekman.history("dead:1").filter((e) => e.type === "rejected")
  ...
  it("refuses an undeclared trigger type when a trigger list is declared", async () => {
    ...
    expect(await code(ekman.send("strict:1", { type: "sneak" }))).toBe("UNKNOWN_TRIGGER")

    const rejected = ekman.history("strict:1").filter((e) => e.type === "rejected")
    expect(rejected).toHaveLength(1)
  ...
  it("accepts any trigger type when none are declared", async () => {
    const ekman = new Ekman({ entities: [orders] })
    await expect(ekman.send("orders:1", { type: "approve", actor: "amy" })).resolves.toBeDefined()
  })
...
describe("sequence and events", () => {
  it("advances the sequence exactly once per commit, with no gaps", async () => {
    const counter = defineEntity("counter", { initial: "a", states: { a: () => stay() } })
    const ekman = new Ekman({ entities: [counter] })

    for (let i = 0; i < 5; i += 1) await ekman.send("counter:1", { type: "tick" })

    expect(transitions(ekman.history("counter:1")).map((e) => e.seq)).toEqual([0, 1, 2, 3, 4, 5])
  })
  ...
  it("honours a caller-supplied trigger id", async () => {
    const ekman = new Ekman({ entities: [orders] })
    await ekman.send("orders:1", { type: "approve", actor: "amy", id: "msg-991" })
    expect(transitions(ekman.history("orders:1"))[1]?.cause.id).toBe("msg-991")
  })

  it("stamps events from the injected clock", async () => {
    const ekman = new Ekman({ entities: [orders], now: steppingClock(1000, 1000) })
    await ekman.send("orders:1", { type: "approve", actor: "amy" })

    expect(transitions(ekman.history("orders:1")).map((e) => e.at)).toEqual([1000, 2000])
  })
  ...
  it("refuses values that are not serializable", async () => {
    ...
    expect(await code(ekman.entities.bad.send("1", { type: "go" }))).toBe("HANDLER_FAILED")
  })
})
...
describe("per-key serialization", () => {
  it("runs one handler at a time per key", async () => {
    ...
    const slow = defineEntity("slow", {
      ...
      states: {
        a: async (instance) => {
          ...
          active -= 1
          return stay({ n: ((instance.values["n"] as number | undefined) ?? 0) + 1 })
        },
      },
    })
    ...
    expect(ekman.entities.slow.inspect("1")).toMatchObject({ seq: 3, values: { n: 3 } })
  })
  ...
  it("keeps the queue alive after a failed trigger", async () => {
    const flaky = defineEntity("flaky", {
      ...
      states: {
        a: (instance, trigger) =>
          trigger.type === "bad" ? fail(new Error("boom")) : stay({ ok: true, ...instance.values }),
      },
    })
...
describe("post", () => {
  it("delivers without awaiting", async () => {
    ...
    await vi.waitFor(() => {
      expect(ekman.entities.orders.inspect("1")).toMatchObject({ state: "approved" })
    })
  })
  ...
  it("routes a failure to onUnhandled rather than dropping it", async () => {
    ...
    expect((onUnhandled.mock.calls[0]?.[0] as EkmanError).code).toBe("HANDLER_FAILED")
  })
})
...
describe("uncovered surface", () => {
  it("reads history through the handle", async () => {
    ...
    expect(ekman.entities.orders.history("1")).toEqual(ekman.history("orders:1"))
  ...
  it("reads a snapshot through the handle", async () => {
    const ekman = new Ekman({ entities: [orders] })
    await ekman.entities.orders.send("1", { type: "approve", actor: "amy" })
    expect(ekman.entities.orders.inspect("1")).toEqual(ekman.inspect("orders:1"))
  })
  ...
  it.each([
    ...
    const ekman = new Ekman({ entities: [orders] })
    expect(await code(ekman.send("orders:1", trigger as unknown as { type: string }))).toBe(
      "UNKNOWN_TRIGGER",
    )
  })
  ...
  it("rejects a trigger whose type is an empty string", async () => {
    const ekman = new Ekman({ entities: [orders] })
    expect(await code(ekman.send("orders:1", { type: "" }))).toBe("UNKNOWN_TRIGGER")
  })
...
describe("remaining edges", () => {
  ...
  it("reports the original failure when an error handler returns a non-result", async () => {
    ...
    expect(ekman.entities.confused.inspect("1")).toMatchObject({ state: "a", seq: 0 })
  })
  ...
  it.each([
    ...
    expect(await code(ekman.entities.wrong.send("1", { type: "go" }))).toBe("HANDLER_FAILED")
    expect(ekman.entities.wrong.inspect("1")).toMatchObject({ state: "a", seq: 0 })
  })
