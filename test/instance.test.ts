...
describe("InstanceRecord", () => {
  it("starts at sequence zero with an initialization event", () => {
    ...
    expect(instance.events[0]).toMatchObject({ from: null, to: "pending", seq: 0, at: 1000 })
  })

  it("exposes a frozen snapshot", () => {
    const snapshot = make({ a: 1 }).snapshot()

    expect(snapshot).toEqual({ key: "orders:1", entity: "orders", state: "pending", values: { a: 1 }, seq: 0 })
  ...
  it("advances the sequence by exactly one per commit", () => {
    ...
    instance.commit({ state: "approved", values: {}, at: 2000, cause: { type: "approve", id: "t2" } })
    instance.commit({ state: "shipped", values: {}, at: 3000, cause: { type: "ship", id: "t3" } })
  ...
  it("records the state it moved from", () => {
    const instance = make()
    const event = instance.commit({ state: "approved", values: {}, at: 2000, cause: { type: "a", id: "t2" } })
    expect(event.from).toBe("pending")
  })

  it("records a rejection without advancing the sequence", () => {
    const instance = make()
    instance.commit({ state: "approved", values: {}, at: 2000, cause: { type: "a", id: "t2" } })
    instance.reject({ code: "UNKNOWN_STATE", reason: "no handler", at: 3000, cause: { type: "x", id: "t3" } })

    expect(instance.seq).toBe(1)
    expect(instance.events).toHaveLength(3)
    expect(instance.events[2]).toMatchObject({ type: "rejected", seq: 1, code: "UNKNOWN_STATE" })
  })

  it("keeps seq non-decreasing across a stream that mixes commits and rejections", () => {
    const instance = make()
    instance.reject({ code: "UNKNOWN_TRIGGER", reason: "x", at: 1, cause: { type: "x", id: "t2" } })
    instance.commit({ state: "approved", values: {}, at: 2, cause: { type: "a", id: "t3" } })
    instance.reject({ code: "UNKNOWN_STATE", reason: "y", at: 3, cause: { type: "y", id: "t4" } })
  ...
  it("leaves only transition events for replay", () => {
    const instance = make()
    instance.commit({ state: "approved", values: {}, at: 2, cause: { type: "a", id: "t2" } })
    instance.reject({ code: "UNKNOWN_STATE", reason: "y", at: 3, cause: { type: "y", id: "t3" } })
