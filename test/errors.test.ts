...
import { composeStack, EkmanError, ERROR_CODES, isEkmanError } from "../src/errors.js"
...
describe("isEkmanError", () => {
  ...
  it("narrows to the code without a cast", () => {
    ...
    expect(isEkmanError(thrown) ? thrown.code : "not-ekman").toBe("UNKNOWN_STATE")
  })
})
...
describe("stack capture on engines without captureStackTrace", () => {
  it("still produces a stack headed by the error name", () => {
    ...
    const ctor = Error as unknown as { captureStackTrace?: Capturer | undefined }
...
describe("composeStack", () => {
  ...
  it("returns just the header when the engine gave no stack at all", () => {
    expect(composeStack("EkmanError", "bad key", undefined)).toBe("EkmanError: bad key")
  })
