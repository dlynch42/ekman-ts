...
export interface TransitionToResult<S extends string = string, V extends Values = Values> {
...
export type HandlerResult<S extends string = string, V extends Values = Values> =
 ...
 */
export function transitionTo<S extends string>(state: S): TransitionToResult<S, never>
...
export function transitionTo<S extends string, V extends Values = Values>(
  ...
  return Object.freeze(
    values === undefined ? { kind: "transitionTo" as const, state } : { kind: "transitionTo" as const, state, values },
  )
}
...
export function stay<V extends Values = Values>(values?: V): StayResult<V> {
  return Object.freeze(values === undefined ? { kind: "stay" as const } : { kind: "stay" as const, values })
}
...
export function toError(value: unknown): Error {
  ...
  return new Error(`handler failed with a non-error value: ${describe(value)}`, { cause: value })
}
