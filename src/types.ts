 ...
 * to change them.
 */
export interface InstanceSnapshot<S extends string = string, V extends Values = Values> {
  readonly key: string
...
export type ErrorHandler<S extends string = string, V extends Values = Values> = (
  instance: InstanceSnapshot<S, V>,
  error: Error,
  ctx: HandlerContext,
) => HandlerResult<S, V> | Promise<HandlerResult<S, V>>
...
export interface CommitResult<S extends string = string, V extends Values = Values> {
  readonly key: string
  readonly state: S
  readonly values: Readonly<V>
  readonly seq: number
...
type HandleFor<D> =
  D extends EntityDefinition<string, infer S, infer V, infer T> ? EntityHandle<S, V, T> : never
